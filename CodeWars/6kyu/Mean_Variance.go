...
func spl(town, strng string) ([]float64, error) {
	townS := strings.Split(strng, "\n")
	strngPlus := make(map[string]string)
	for _, val := range townS {
		s := strings.Split(val, ":")
		if len(s) == 2 {
			strngPlus[s[0]] = s[1]
		}
	}
	data, found := strngPlus[town]
	if !found {
		return nil, errors.New("town not found")
	}
	month := strings.Split(data, ",")
	var pr []float64
	for _, val := range month {
		sp := strings.Split(val, " ")
		if len(sp) == 2 {
			r, err := strconv.ParseFloat(sp[1], 64)
			if err != nil {
				return nil, err
			}
			pr = append(pr, r)
